...
public class MapMover extends Sprite
	{
        ...
        private var _movingSprite:Rectangle;
        ...
        private var stageCentralPoint:Point = new Point(Platformer.WIDTH/2, Platformer.HEIGHT/2);
		...
		*/
		...
		/* CONSTRUCTOR */
		public function MapMover (stage:Stage, target:DisplayObject, movingSprite:Rectangle,
								  fixPoint:Point, speedFactor:Number)
		{
			_target = target;
			_stage = stage;
            _movingSprite = movingSprite;
            _levelWidth = _movingSprite.width;
            _levelHeight = _movingSprite.height;
			_fixPoint = fixPoint;
		...
		private function checkBordersCollission():void
		{
			...
			if(_movingSprite.x + _levelWidth <= _stage.stageWidth)
			{
...
//			if(_stage.y + _stage.stageHeight >=  _stage.height)
			if(_stage.y + _stage.stageHeight >= _movingSprite.height)
			{
		...
		{
            _tweenX = new TweenMax(_movingSprite, 0.5, {x:distX});
        ...
        {
            _tweenY = new TweenMax(_movingSprite, 0.5, {y:distY});
